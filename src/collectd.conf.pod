...
The plugin is able to get exceptions data from two possible sources.
First one is mcelog daemon unix socket that provides aggregated stats of memory
related exceptions. Default socket file handle specified in
B<McelogClientSocket> is B<"/var/run/mcelog-client">.
For that data source mcelog plugin submits aggregated errors as values
as well as related notifications.

Second possible data source that provides more detailed information related
not only to memory, but also to IO and CPU exceptions is mcelog daemon
output log file that may be enabled in mcelog configuration. Default path
is configured in E<lt>B<McelogLogFile>E<gt> as B<"/var/log/mcelog">. This data
source requires mandatory set of regular expressions that define rules
for filtering out MCE related messages from source log file, which could be
a dedicated log file or syslog. Parsed exceptions contain detailed MCE
information in text format and are submitted as collectd notifications.
Base set of regular expressions for parsing MCE is provided in B<McelogLogFile>
markup.

Both data sources can be handled simultaneously. In order to disable any of
them, corresponding config section should be commented out.
...
=item B<McelogClientSocketEnabled> B<true>|B<false>

Setting to B<true> enables parsing memory MCE related info from mcelog socket.
Defaults to B<true>.

=item B<McelogLogfile> I<Path>

The mcelog file to parse. Defaults to B<"/var/log/mcelog">. It can be any file
containing message lines matching the regular expression given in
E<lt>B<Match>E<gt> subsections explained below.

=item E<lt>B<Match>E<gt>

Multiple B<Match> blocks define regular expression patterns for extracting or
excluding specific string patterns from parsing.
First and last E<lt>B<Match>E<gt> items set boundaries of multiline
MCE message and are mandatory.

Special B<Match> block with I<ORIGIN> B<Name> can be added between mandatory
B<Match> blocks. The purpose of it is to allow a user to select a string from
multiline MCE message, that will be used as a plugin instance for dispatched
notifications. This block applies only to single multiline MCE message rule.
Other notifications' plugin instances will be unaffected. If regular expression
doesn't match any parsed line, plugin instance will be set to I<other>.

=item B<Regex> I<regex>

Regular expression with pattern matching string that should be included in MCE
content. It may contain subexpressions, so next option B<SubmatchIdx> specifies
which subexpression should be stored as a part of MCE message.

=item B<SubmatchIdx> I<index>

Index of subexpression to be stored as part of MCE message. Multiple
...
This is processed before checking B<Regex> pattern.

=item B<IsMandatory>  I<true>|I<false>

Flag specifying if message item string described by above parameters is
mandatory for whole message validation. If set to 1, whole message is discarded
if it's missing. For 0 it's presence in MCE message is optional.

=item B<McelogLogfileEnabled> B<true>|B<false>

Setting to B<true> enables parsing all MCE related data from log file.
Defaults to B<true>.
